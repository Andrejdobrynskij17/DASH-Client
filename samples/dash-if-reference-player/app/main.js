...
app.controller('DashController', ['$scope', '$window', 'sources', 'contributors', 'dashifTestVectors', function ($scope, $window, sources, contributors, dashifTestVectors) {
    ...
    $scope.chartState = {
        audio: {
            ...
            bufferZero: { data: [], selected: false, color: '#00ffff', label: 'Buffer Underflow' },
            qualityChange: { data: [], selected: false, color: '#0000ff', label: 'Quality Change' },
        },
        video: {
            ...
            liveLatency: { data: [], selected: false, color: '#65080c', label: 'Live Latency' },
            playbackRate: { data: [], selected: false, color: '#65080c', label: 'Playback Rate' },
            bufferZero: { data: [], selected: true, color: '#00ffff', label: 'Buffer Underflow' },
            qualityChange: { data: [], selected: true, color: '#0000ff', label: 'Quality Change' },
        }
    };
    ...
    $scope.videoBufferZero = 0;
    $scope.videoQualityChange = 0;
    $scope.videoBufferLengthSum = 0;
    $scope.bitrateSum = 0;
    $scope.startTime = 0;
    $scope.timeCounts = 0;
    $scope.endFlag = 0;
    ...
    $scope.audioBufferZero = 0;
    $scope.audioQualityChange = 0;
    ...
    $scope.player.on(dashjs.MediaPlayer.events.BUFFER_EMPTY, function (e) { /* jshint ignore:line */
        $scope.videoBufferZero += 1;
    }, $scope);

    $scope.player.on(dashjs.MediaPlayer.events.PLAYBACK_STARTED, function (e) { /* jshint ignore:line */
        console.log("!!!!!Playback started!!!!!")
        $scope.startTime = new Date().getTime() / 1000 - $scope.sessionStartTime;
        console.log("!!!!!Time to start: " + $scope.startTime + "!!!!!")
    }, $scope);
    ...
    $scope.player.on(dashjs.MediaPlayer.events.PLAYBACK_ENDED, function (e) { /* jshint ignore:line */
        ...
        console.log("Stalls: " + $scope.videoBufferZero);
        console.log("Quality changes: " + $scope.videoQualityChange);
        console.log("Avg. bitrate: " + $scope.bitrateSum / $scope.timeCounts);
        console.log("Avg. buffer length: " + $scope.videoBufferLengthSum / $scope.timeCounts);
        $scope.endFlag = 1;
    }, $scope);
    ...
    function updateMetrics(type) {
        ...
        if (dashMetrics && $scope.currentStreamInfo) {
            ...
            if(type == 'video'){
                console.log("!!!!!!!!!!!! LOGGING !!!!!!!!!!!!");
                var vidbufferZero = $scope.videoBufferZero;
                var vidqualityChange = $scope.videoQualityChange;
            }
            ...
            if(type == 'video'){
                $scope[type + 'BufferZero'] = vidbufferZero;
                $scope[type + 'QualityChange'] = vidqualityChange;
                $scope[type + 'BufferLengthSum'] += bufferLevel;
                $scope.bitrateSum += bitrate;
                $scope.timeCounts += 1;
            }
            ...
            if(type == 'video' && $scope.endFlag == 0){
                console.log("Start time: " + $scope.startTime);
                console.log("Stalls: " + vidbufferZero);
                console.log("Quality changes: " + vidqualityChange);
                console.log("Bitrate: " + bitrate);
                console.log("Buffer length: " + bufferLevel);
            }
            ...
            if ($scope.chartCount % 2 === 0) {
                ...
                if(type == 'video'){
                    $scope.plotPoint('bufferZero', type, vidbufferZero, time);
                    $scope.plotPoint('qualityChange', type, vidqualityChange, time);
                }
                if (httpMetrics) {
